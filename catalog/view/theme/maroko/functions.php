...
function road_mega_menu_id_change_call_back($html){
	global $road_opt, $mega_main_menu;
	
	if(isset($road_opt)){
		if ( ($road_opt['header_layout']!='second') && !is_page_template( 'page-templates/demo2.php' )){
		...
		$html = preg_replace('/id="mega_main_menu"/', 'id="mega_main_menu_first"', $html);
		$html = preg_replace('/id="mega_main_menu_ul"/', 'id="mega_main_menu_ul_first"', $html);
		
		$html = preg_replace('/id="mega_main_menu_first"/', 'id="mega_main_menu"', $html, 1);
		$html = preg_replace('/id="mega_main_menu_ul_first"/', 'id="mega_main_menu_ul"', $html, 1);
	} */
...
function road_setup() {
	...
	register_nav_menu( 'topmenu', __( 'Top Menu', 'roadthemes' ) );
	...
	add_image_size( 'post-thumb', 500, 500, true ); // (cropped)
	add_image_size( 'post-thumbwide', 570, 310, true ); // (cropped)
}
	...
	*/
	...
	if($road_opt['enable_less']){
		...
		switch ($presetopt) {
			...
			case 3:
				$themevariables['primary_color'] = '#fa713a';
			break;
		...
		if(function_exists('compileLessFile')){
			...
			if ( is_rtl() ) {
				compileLessFile('rtl.less', 'rtl'.$presetopt.'.css', $themevariables);
			}
	...
	//load rtl css
	if ( is_rtl() ) {
		wp_enqueue_style( 'roadthemes-rtl', get_template_directory_uri() . '/css/rtl'.$presetopt.'.css', array(), '1.0.0' );
	}
	//Compare CSS
...
function road_widgets_init() {
	...
	register_sidebar( array(
		'name' => __( 'Sub Sidebar', 'roadthemes' ),
		'id' => 'sidebar-sub',
		'description' => __( 'Sidebar on the right side', 'roadthemes' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title"><span>',
		'after_title' => '</span></h3>',
...
function road_meta_box_callback( $post ) {
	...
	_e( 'W `P `L ` O `C `K `E `R`.`C `O `M` - This content will be used to replace the featured image, use shortcode here', 'roadthemes' );
	echo '</label><br />';
